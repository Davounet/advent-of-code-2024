const input = `Button A: X+63, Y+14
Button B: X+12, Y+37
...
const machines = input.split('\n\n').map(block => {
  ...
  const matches = /.+X\+(\d+).+Y\+(\d+).+X\+(\d+).+Y\+(\d+).+X=(\d+).+Y=(\d+)/g.exec(raw)
  const m = matches.map(Number)
  return { A: [m[1], m[2]], B: [m[3], m[4]], prize: [m[5], m[6]] }
})

const combinations1 = machines.map(getCombinations1)
const lowestCosts1 = combinations1.filter(c => c.length).map(getLowestCost)
const total1 = lowestCosts1.reduce((s, c) => s + c, 0)
console.log({ total1 })

const combinations2 = machines.map(getCombinations2)
const lowestCosts2 = combinations2.filter(c => c.length).map(getLowestCost)
const total2 = lowestCosts2.reduce((s, c) => s + c, 0)
console.log({ total2 })

// Brute force
function getCombinations1({ A, B, prize }) {
  const possibles = Array.from({ length: 100 }).map((_, i) => {
    const j = (prize[0] - A[0] * i) / B[0]

    if (!Number.isInteger(j)) return false
    if (A[1] * i + B[1] * j !== prize[1]) return false
    return [i, j]
  })
  return possibles.filter(Boolean)
}

// System of 2 equations
function getCombinations2({ A, B, prize }) {
  const target = [10000000000000 + prize[0], 10000000000000 + prize[1]]

  const delta = A[1] * B[0] - A[0] * B[1]
  if (delta === 0) return []

  const j = (target[0] * A[1] - target[1] * A[0]) / delta
  const i = (target[0] - B[0] * j) / A[0]

  if (!Number.isInteger(i) || !Number.isInteger(j)) return []
  return [[i, j]]
}

function getLowestCost(combinations) {
  const costs = combinations.map(([a, b]) => 3 * a + b)
